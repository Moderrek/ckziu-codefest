...
const Project = (props: { project: ApiProjectData }) => {
  ...
  return (
    <div className='max-w-sm overflow-hidden rounded-t-lg rounded-b border-b-4 border-r-4 shadow-2xl'>
      ...
      <div className='px-6 py-4'>
        <div className='flex flex-row text-2xl font-bold justify-between'>
          <Link href={`/p/${project.author}/${project.display_name}`} className="hover:underline">
            {project.display_name}
          </Link>
          <div className='ml-5 flex flex-row justify-between text-right select-none cursor-pointer' onClick={() => {
            setLiked(!liked)
        ...
        <p className='text-base font-bold'>
          ...
          <HoverCard openDelay={1}>
            <HoverCardTrigger asChild>
              <Link href={`/p/${project.author}`} className="hover:underline flex flex-row items-center">
